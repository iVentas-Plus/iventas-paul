...
export function registerAdminTaskWriteTool(server, admin) {
    server.registerTool("paul_admin_task_write", {
        ...
        try {
            return await runTaskWrite(admin, args);
        }
        catch (err) {
...
async function runTaskWrite(admin, args) {
    ...
    if (action === "update") {
        ...
        if (!args.title) {
            return missingFieldResult("update needs `title`: the panel's edit form posts every field at once, so " +
    ...
    const html = action === "delete" ? await admin.deleteTask(args.id) : await admin.completeTask(args.id);
    return taskWriteResult(action, html, { id: args.id });
}
