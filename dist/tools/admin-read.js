...
import { parseHeadings, parseKpis, parseTables, parseAdminTasks, parseSelectOptions, stripTags, } from "../admin-parse.js";
...
export function truncateText(html) {
    const text = stripTags(html);
    if (text.length <= MAX_TEXT_CHARS)
        return { text };
    return {
        text: text.slice(0, MAX_TEXT_CHARS),
        note: `TRUNCATED: the page's text is ${text.length} characters and only the first ` +
...
function unknownPageResult(page) {
    return textResult({
        error: true,
        message: `"${page}" is not an admin page of PAUL. No request was made. Valid pages: ` +
            `${ADMIN_PAGES.join(", ")}.`,
    }, true);
}
const STATUS_DESCRIPTION = "Read one of PAUL's TEAM-WIDE admin pages — the ones that show every " +
...
export function registerAdminStatusTool(server, admin) {
    server.registerTool("paul_admin_status", {
        ...
        inputSchema: {
            ...
            params: z
                .record(z.string())
                .optional()
                .describe("Query string parameters, e.g. {p:'week'} for pulse, {ym:'2026-08'} " +
...
const TASKS_DESCRIPTION = "Read a collaborator's mission board from the admin panel. This is the ONLY " +
...
export function registerAdminTasksTool(server, admin) {
    server.registerTool("paul_admin_tasks", {
...
async function sweepAllBoards(admin) {
    ...
    if (roster.length === 0) {
        return {
            error: true,
            message: "admin/tasks.php did not expose its user list, so there is no roster to " +
...
export function registerAdminPageTool(server, admin) {
    server.registerTool("paul_admin_page", {
...
const ASK_DESCRIPTION = "Ask PAUL's own copilot a natural-language question over the WHOLE admin " +
    ...
    "acting on it.";
export function registerAdminAskTool(server, admin) {
    server.registerTool("paul_admin_ask", {
