...
export function registerTipsTools(server, client) {
    server.registerTool("paul_tips", {
        title: "Read PAUL's coaching tips",
        description: "Read the tips PAUL emits. IMPORTANT: tips are written BY PAUL for the " +
        ...
        try {
            const res = taskId === undefined ? await client.paulTips() : await client.taskTips(taskId);
