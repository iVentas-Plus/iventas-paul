...
async function runAction(
  ...
  switch (action) {
    ...
    case "reassign":
      ...
      if (!reason) {
        throw new Error(
          "action 'reassign' needs `reason`: PAUL forwards it to the admins and learns from it.",
        );
      }
...
export function registerTaskActionTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_task_action",
    {
      title: "Act on an existing PAUL task",
      description:
        "Run a lifecycle action on a task that already exists. Actions: " +
    ...
    async ({ id, action, toUid, reason }) => {
      try {
        const res = (await runAction(client, action, id, toUid, reason)) as Record<
          string,
          unknown
        >;
...
export function registerUndoAssignmentTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_undo_assignment",
    {
      title: "Undo a task you just created",
      description:
        "Delete a task that was just created, using the taskId returned by " +
