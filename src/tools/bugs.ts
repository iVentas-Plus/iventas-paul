...
import type { PaulClient, Bug, PeerContact } from "../client.js";
import { textResult, errorResult } from "./shared.js";

/** Statuses the bug board always reports, in board order (api.php bugs_list). */
const BUG_STATUSES = ["abierto", "en_proceso", "resuelto"] as const;
...
function summarize(bug: Bug) {
  const notes = safeArray<{ who: string; note: string | null; images: unknown }>(bug.notes);
  return {
    id: bug.id,
    title: bug.title,
    status: bug.status,
    ...
    notes: notes.map((n) => ({
      who: n.who ?? null,
      note: n.note ?? null,
      images: safeArray<string>(n.images),
    })),
  };
}

export function registerBugsTools(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_bugs",
    {
      title: "List PAUL bug reports",
      description:
        "List every bug reported against PAUL itself by the team, grouped by " +
    ...
    async () => {
      try {
        ...
        const bugs = safeArray<Bug>(data?.bugs);
        const counts: Record<string, number> = {};
        const grouped: Record<string, ReturnType<typeof summarize>[]> = {};
        for (const status of BUG_STATUSES) grouped[status] = [];
  ...
  server.registerTool(
    "paul_report_bug",
    {
      ...
      description:
        "Report a bug in PAUL itself (the coach app), not in the user's own " +
  ...
  server.registerTool(
    "paul_assign_bug",
    {
      ...
      description:
        "Assign an open bug to a teammate with an urgency level. `personUid` " +
