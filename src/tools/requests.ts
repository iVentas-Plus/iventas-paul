...
const REQ_STATUSES = ["nueva", "asignada", "hecha", "descartada"] as const;

const KINDS = ["bug", "idea", "mejora", "soporte"] as const;
const MONEY_KINDS = ["gana", "ahorra", "otro"] as const;
const MONEY_OTHERS = [
  "cliente_grande",
  "cliente_renovar",
  "cliente_importante",
...
const URGENCIES = ["urgente", "alta", "media", "baja"] as const;
const ACTIONS = ["take", "assign", "done", "discard"] as const;
...
function summarize(req: PaulRequest) {
  return {
    id: req.id,
    kind: req.kind ?? null,
 ...
 * must not rely on.
 */
function ambiguousFirstNames(roster: RequestRosterEntry[]): string[] {
...
async function runAction(
  ...
  switch (action) {
    case "take":
      return (await client.reqTake(id)) as unknown as Record<string, unknown>;
    case "assign":
      if (!personUid) {
        throw new Error(
          "action 'assign' needs `personUid` — the uid of the person who will do it, " +
            "taken from the `team` array of paul_requests or from paul_people.",
        );
      }
      return (await client.reqAssign(id, personUid, urgency)) as unknown as Record<
        string,
        unknown
      >;
    case "done":
      return (await client.reqStatus(id, "hecha")) as Record<string, unknown>;
    case "discard":
      if (!reason) {
        throw new Error(
          "action 'discard' needs `reason`: PAUL shows it to whoever filed the request, " +
            "and a discard with no explanation is what makes people stop filing them.",
        );
      }
      return (await client.reqStatus(id, "descartada", reason)) as Record<string, unknown>;
  }
}


/** Registers the queue listing. */
function registerRequestsListTool(server: McpServer, client: PaulClient): void {
server.registerTool(
  "paul_requests",
  {
    title: "List PAUL's team request queue (Peticiones)",
    description:
      "List PAUL's 'Peticiones' queue: the ONE place the team files bugs, " +
  ...
  async () => {
    try {
      ...
      const reqs = safeArray<PaulRequest>(data?.reqs);
      const counts: Record<string, number> = {};
      const grouped: Record<string, ReturnType<typeof summarize>[]> = {};
      for (const status of REQ_STATUSES) grouped[status] = [];
      ...
      return textResult({
        ...
        team: safeArray<PeerContact>(data?.team).map((p) => ({ uid: p.uid, name: p.name })),
        requests: grouped,
      });
    } catch (err) {
...
function registerCreateRequestTool(server: McpServer, client: PaulClient): void {
server.registerTool(
  "paul_create_request",
  {
    title: "File a request in PAUL (Peticiones)",
    description:
      "File a request in PAUL's team queue. This is the CURRENT way to ask " +
  ...
  async (args) => {
    ...
    try {
      ...
      if ((migrateSrc === undefined) !== (migrateId === undefined)) {
        throw new Error(
          "migrateSrc and migrateId go together: pass both to migrate a historical " +
            "bug/idea row, or neither to file a brand-new request.",
        );
      }
      const res = await client.reqCreate({
        kind: kind as ReqKind,
        title,
        detail,
        moneyKind: moneyKind as ReqMoneyKind,
        moneyOther: moneyOther as ReqMoneyOther | undefined,
        moneyMonth: usdPerMonth,
        monthsMin: minMonths,
        urgency: urgency as ReqUrgency,
        migrateSrc,
        migrateId,
      });
      return textResult({
...
function registerRequestThreadTool(server: McpServer, client: PaulClient): void {
server.registerTool(
  "paul_request_thread",
  {
    title: "Read a request's discussion thread",
    description:
      "Read one request from paul_requests plus its whole comment thread. " +
  ...
  async ({ id }) => {
    try {
      ...
      const roster = safeArray<RequestRosterEntry>(data?.roster);
      return textResult({
        ok: data?.ok ?? true,
        request: data?.req ?? null,
        comments: safeArray<RequestComment>(data?.comments),
        roster,
...
function registerCommentRequestTool(server: McpServer, client: PaulClient): void {
server.registerTool(
  "paul_comment_request",
  {
    title: "Comment on a request's thread",
    description:
      "Post a comment on a request's thread. Anyone may comment, on any " +
...
function registerRequestActionTool(server: McpServer, client: PaulClient): void {
server.registerTool(
  "paul_request_action",
  {
    title: "Take, assign, close or discard a request",
    description:
      "Move a request through its lifecycle. Actions: 'take' claims a `nueva` " +
 ...
 */
export function registerRequestsTools(server: McpServer, client: PaulClient): void {
