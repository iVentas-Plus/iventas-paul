...
export interface AssignResult {
  ok: boolean;
  /** The new task's id. `assign_confirm` returns it as `undo_id`. */
  taskId?: number;
  /** Display name of the person the task landed on, when PAUL reports it. */
  assignedTo?: string;
  /**
   * true when the autopilot is off for that person's department: PAUL did not
   * create the task, it queued the proposal for an admin to confirm. The work
   * is NOT assigned yet.
   */
  queued?: boolean;
  /** Human-readable explanation of a non-obvious outcome, set for `queued`. */
  note?: string;
  /** PAUL's own reply, verbatim, so the caller can relay it. */
...
export async function assignTask(
  client: Pick<PaulClient, "assignConfirm">,
  input: {
    title: string;
    urgency: Urgency;
    personUid: string;
  ...
  return {
    ...
    ...(res.queued
      ? {
          queued: true,
          note:
            "PAUL did NOT create the task: the autopilot is off for that " +
...
export function registerRegisterTaskTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_register_task",
    {
      ...
      description:
        "Create a NEW task in your OWN list in PAUL. One direct API call — no " +
        ...
        return textResult(
          {
            ok: false,
            error:
              "PAUL did not report your uid on login, so the task cannot be " +
...
export function registerAssignTaskTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_assign_task",
    {
      title: "Assign a task to another person in PAUL",
      description:
        "Create a task in ANOTHER collaborator's list. `personUid` is PAUL's " +
