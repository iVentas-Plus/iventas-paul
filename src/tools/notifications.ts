...
import type { PaulClient, PaulNotification } from "../client.js";
 ...
 * paul_request_thread.
 */
function requestIdOf(ref: unknown): number | null {
  if (typeof ref !== "string" && typeof ref !== "number") return null;
  const parsed = Number(ref);
  return Number.isInteger(parsed) && parsed > 0 ? parsed : null;
}

export function registerNotificationsTool(server: McpServer, client: PaulClient): void {
  server.registerTool(
    "paul_notifications",
    {
      title: "Read PAUL's notification bell",
      description:
        "Read the user's PAUL notifications: @mentions and new activity on the " +
    ...
    async ({ markSeen }) => {
      try {
        ...
        const notifs = safeArray<PaulNotification>(data?.notifs);
        let marked = false;
        if (markSeen === true) {
          try {
